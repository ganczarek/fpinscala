...
sealed abstract class STArray[S,A](implicit manifest: Manifest[A]) {
  ...
  def fill(xs: Map[Int, A]): ST[S, Unit] =
    xs.foldLeft(ST[S, Unit](()))((st, kv) => st flatMap (_ => write(kv._1, kv._2)))
...
object Immutable {
  ...
  def partition[S](a: STArray[S, Int], l: Int, r: Int, pivot: Int): ST[S, Int] = for {
    ...
    _ <- a.swap(pivot, r)
    x <- swampIfValueLessThanPivot(a, l, r, pivotVal)
    _ <- a.swap(x, r)
  } yield x

  def swampIfValueLessThanPivot[S](a: STArray[S, Int], l: Int, r: Int, pivotVal: Int): ST[S, Int] =
    for {
      j <- STRef(l)
